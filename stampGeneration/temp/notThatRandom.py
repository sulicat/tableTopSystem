...
def imageRandomizeImageSet( image, fill, path ):
    w, h, c = image.shape[:3]
    ...
    image = cv.imread( path +  files[index] )
    image = cv.cvtColor( image, cv.COLOR_BGR2GRAY )
    ret,tempImage = cv.threshold( image, 100, 255, cv.THRESH_BINARY )
    tempImage = cv.resize(tempImage, (w, h) )
    image = tempImage
    return image
...
#    output = imageGradient( output, fill )
    output = imageRandomizeImageSet( output, fill, "../resources/randomImages/" )
