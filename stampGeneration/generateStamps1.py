...
output_w = stampCount_x*(stamp_size + stampPadding) + stampPadding
output_h = math.floor((stamp_count / stampCount_x) + 1) * (stamp_size + stampPadding) + stampPadding
...
def drawStamp( img, pos, r, fill, idNum, bits ):
    ...
    for i in range( 0, bits ):
        if( (idNum >> i & 1) == 1 ):
            ...
            x2 = int( x2 + pos[0])
            y2 = int( y2 + pos[1])
            cv.line( img, (x1,y1),(x2,y2),fill, 8)
